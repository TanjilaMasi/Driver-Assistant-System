...
def main():
    ...
    Predictor = dlib.shape_predictor(
        "E:/PROJECT/Driver-State-Detection-master/predictor/shape_predictor_68_face_landmarks.dat")  # instantiation of the dlib keypoint detector model
    '''
        ...
        if delta_time >= time_lim:  # if the time passed is bigger or equal than the frame time, process the frame
            ...
            if len(faces) > 0:  # process the frame only if at least a face is found
                ...
                '''if tired:
                    ...
                    pygame.mixer.music.load('E:/PROJECT/Driver-State-Detection-master/Alarm.mp3')
            ...
            # stop the tick counter for cv2.imshow("Press 't' to terminate", frame) computing the processing time for each frame
            e2 = cv2.getTickCount()
            # processign time in milliseconds
            proc_time_frame_ms = ((e2 - e1) / cv2.getTickFrequency()) * 1000
            # print fps and processing time per frame on screen
            if args.show_fps:
                cv2.putText(frame, "FPS:" + str(round(fps, 0)), (10, 500), cv2.FONT_HERSHEY_PLAIN, 2,
                            (255, 0, 255), 1)
            if args.show_proc_time:
                cv2.putText(frame, "PROC. TIME FRAME:" + str(round(proc_time_frame_ms, 0)) + 'ms', (10, 530), cv2.FONT_HERSHEY_PLAIN, 2,
                            (255, 0, 255), 1)

            # show the frame on screen
            cv2.imshow("Press 't' to terminate", frame)
